...
export default class RoleEdit extends React.Component {
    ...
    componentDidMount() {
        let { id } = this.state;
        App.api('merchant/adm/permissions').then((permissions) => {
            permissions = permissions.filter(item => item.key !== 'NONE');
            this.setState({
                permissions
            });
            if (id !== 0) {
                App.api('merchant/adm/role', { id }).then((role) => {
                    this.setState({
    ...
    handleSubmit = () => {
        ...
        App.api('merchant/adm/save_merchantRole', { role: JSON.stringify(role) }).then((res) => {
            if (id !== 0) {
                App.logout();
                App.go("/login");
